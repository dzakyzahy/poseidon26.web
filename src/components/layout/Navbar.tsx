...
const Navbar = () => {
  ...
  const isVisible = !isHome || isScrolled || isHoveringTop;
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      setProgress(100);
      setTimeout(() => setProgress(0), 400); // hide after complete
    }, 600);
  ...
  return (
    <>
      ...
      <div 
        className="fixed top-0 left-0 h-1 bg-bioluminescent-blue z-[100] transition-all duration-300 ease-out" 
        style={{ 
          width: `${progress}%`, 
          opacity: progress > 0 && progress < 100 ? 1 : (progress === 100 ? 0 : 0) 
        }} 
      />

      {/* Hover Trigger Area for Landing Page */}
      {isHome && !isScrolled && (
        <div 
      ...
      <nav 
        className={`fixed w-full z-50 transition-all duration-500 pointer-events-none ${
